...
才关得掉窗口」，而那两个按钮正好都是解析过程中会按的。改成 actor 后真机确认修好了。
Mac 快到察觉不出，**模拟器复现不了这个问题**，别因为模拟器上一次就能关掉就以为没事。

**AI 面板的预热要让出首次交互。** 面板一出现就同帧加载模型权重 + 发三个课程请求
（`DeadlineRepository` 是 `@MainActor`，响应解码也在主线程），真机上是「刚进去卡一会儿、
按钮没反应、输入框要点好久、最开始打字也卡」。现在预热延后 700ms 且降到 `.utility`；
课程请求不延后只降优先级（大头是挂起的网络等待），但 `parseAI` 必须 `await` 那个 task，
否则「打开就点分析」会静默拿不到课程归属。另外 `SystemLanguageModel.default.availability`
**每次读都有开销**（开发机实测 0.36ms/次），它被写在面板 body 里就等于每个按键付一次，
现在缓存了——别把它改回每次现读。
